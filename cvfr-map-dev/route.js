...
function removeWaypoint(id) {
    ...
    if (route_string != "") {
		...
		console.log(route_string_as_Array);
...
function validateAirport(id) {
    ...
    console.log(id);
    console.log(airport_code.length);
...
function updatePopup(airport_code, if_airport) {
	...
	console.log(route);
    let dep = document.getElementById("departure").innerHTML;
	console.log(dep);
    let arr = document.getElementById("arrival").innerHTML;
	console.log(arr);
    let marker_info = marker.getContent().split("<br>");
	console.log(airport_code);
	if (!if_airport){
		marker_info[3] = marker_info[3].replace("</div>", "");
		
	console.log(marker_info[3]);
	}
    marker_info = marker_info[0] + "<br>" + marker_info[1] + "<br>" + marker_info[2] + "<br>" + marker_info[3];
	
    if (route.includes(airport_code)) {
        marker_info += `<br><button type="button" id="${airport_code}" onclick="removeWaypoint(this.id)" class="marker-button">הסר מנתיב</button>`;
    }
    if (dep != airport_code && if_airport) {
        //הוספת כפתור קבע כשדה המראה
        marker_info += `<br><button type="button" onclick="setAsDeparture(this.dataset.airport)" data-airport="${airport_code}" class="marker-button">קבע כשדה המראה</button>`;
    }
    if (arr != airport_code && if_airport) {
        marker_info += `<br><button type="button" onclick="setAsArrival(this.dataset.airport)" data-airport="${airport_code}" class="marker-button">קבע כשדה נחיתה</button>`;
    }
...
var box_height = div.offsetHeight, box_width = div.offsetWidth, window_height = window.innerHeight, window_width = window.innerWidth;
